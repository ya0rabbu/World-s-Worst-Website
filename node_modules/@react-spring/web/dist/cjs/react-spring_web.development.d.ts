import { CSSProperties, ComponentPropsWithRef, ElementType, ForwardRefExoticComponent, JSX } from "react";
...
type AnimatedPrimitives = { [Tag in Primitives]: AnimatedComponent<Tag> };
...
type AnimatedProps<Props extends object> = { [P in keyof Props]: P extends 'ref' | 'key' ? Props[P] : AnimatedProp<Props[P]> };
type StyleProps = Merge<CSSProperties, TransformProps>;
type StylePropKeys = keyof StyleProps;
type ValidStyleProps<T extends object> = { [P in keyof T & StylePropKeys]: T[P] extends StyleProps[P] ? P : never }[keyof T & StylePropKeys];
...
type AnimatedStyles<T extends ReadonlyArray<any>> = { [P in keyof T]: [T[P]] extends [infer DT] ? DT extends object ? [ValidStyleProps<DT>] extends [never] ? DT extends ReadonlyArray<any> ? AnimatedStyles<DT> : DT : { [P in keyof DT]: AnimatedProp<DT[P]> } : DT : never };
...
type AnimatedObject<T extends object> = { [P in keyof T]: AnimatedStyle<T[P]> } | (T extends ReadonlyArray<number | string> ? FluidValue<Readonly<T>> : never);
...
//#endregion
export { AnimatedComponent, AnimatedProps, type CSS, WithAnimated, animated as a, animated };